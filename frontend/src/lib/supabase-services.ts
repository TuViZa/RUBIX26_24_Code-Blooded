...
// Mock services for advanced features (no Supabase dependency)
export const supabaseServices = {
  ...
  ambulance: {
    ...
    listenToEvents: (callback: (data: any) => void) => {
      ...
      supabaseServices.ambulance.getEvents().then(callback);
  ...
  disease: {
    ...
    listenToOutbreaks: (callback: (data: any) => void) => {
      // Return mock data immediately
      supabaseServices.disease.getOutbreaks().then(callback);
      return () => {}; // No cleanup needed
    }
  },
  ...
  resource: {
    ...
    listenToUsage: (callback: (data: any) => void) => {
      // Return mock data immediately
      supabaseServices.resource.getUsage().then(callback);
      return () => {}; // No cleanup needed
    }
  }
};
